...
# 检查并替换现有命令
if [ -f ~/.local/bin/create-repo ] || [ -f ~/.local/bin/delete-repo ]; then
    echo -e "\033[33m检测到已存在的命令，正在更新...\033[0m"
    rm -f ~/.local/bin/create-repo ~/.local/bin/delete-repo
fi
...
echo -e "\033[32m安装完成！现在可以直接使用 create-repo 和 delete-repo 命令了\033[0m"
